...
* **Vitest** (unit tests, 782 in total)
...
│       ├── equipo/            # The team with their public CV: who is who before booking
...
* **Public stylist CV**, in two places and with **one shared form**: the "CV público" tab on the profile (for the admin, on any professional) and the "Mi CV público" block in "Mi perfil" (for the stylist, with the `PERFIL_CV_EDITAR` permission). Duplicating it would guarantee they drifted apart one day, and with them what counts as a valid specialty. Specialties are added one at a time as tags, and the two things the server rejects are flagged **before spending a request**: a comma inside a tag (it is the separator they are stored with) and the same tag twice. **An empty field clears it**, because the block is replaced whole: it is the only way to remove a bio that no longer applies. The photo goes in its own request and saves when picked, not when "Guardar CV" is pressed: merging them would mean re-uploading the image every time a comma is fixed. The **"Team" order** lives on the profile and not in the CV, because putting yourself first displaces your colleagues
* **The business** (under "Configuration"): what this salon is called, how to reach it, which logo and brand colour it uses, and **what time it opens**. The timetable lived in the server's configuration and the name and phone number were written into the mobile app's code: changing them meant touching the deployment and restarting, or rebuilding and reinstalling the APK. The timetable here is the **fixed weekly one** (opening time, closing time and the weekdays it never opens); public holidays and one-off closures are still "Closed days", which go by date. What is **not** here because it belongs to the installation rather than the business: the email sender and the Stripe keys.
* **Modules** (under "Configuration"): what this salon actually does. There are nine: commissions, payments with their three methods, gallery, team CV, sales and email reminders. A module switched off **disappears for everyone, administrators included** - that is the difference from a permission, which only narrows what a role may do - and the backend answers 409, not 403: it is not "you may not", it is that here it does not exist. Payment methods hang off "Pagos": switching the parent off switches them off, and switching them all off switches the parent off, because charging left "on" with no way to charge would be a screen that lies. Before saving, it spells out **what is lost**: switching payments off changes what sales mean (they start counting appointments that were done even if no payment is recorded), and that cannot be discovered afterwards by staring at a zero. Nothing is deleted: stored rates and frozen history stay where they are. Plus the **start-up profiles**, which get a newly arrived salon's set of modules ready instead of switching them off one by one: a profile describes the complete state - anything outside it is switched off - and the screen shows the list of what is about to be switched off first, because it cannot be one click away.
...
* **`returnUrl` on the login screen**, which is what lets you sign in without losing what you were doing: **the guards set it when they bounce for a missing session** — so a session that expired on "My appointments" returns there and not to the home screen — and so does "El equipo", which is public. It is **validated**, because a destination arriving in a parameter is an open redirect if unchecked: only internal routes are accepted — a single leading `/`, never `//host`, a scheme, or a backslash, which some browsers treat as the other one — and both the login itself (it would loop) and the root (not a destination) are rejected. The validation lives in the core (`rutaInternaSegura`) because both apps need the same thing. **Both** entry doors honour it, the form and the fingerprint. A bounce **by role** deliberately saves no destination: there is no missing session there, that place is simply not theirs, and saving it would only bounce them again. And on sign-in the destination is checked as reachable for that role: obeying blindly would land on the screen the other area's guard is about to reject
...
* **"El equipo"**, with each professional's CV: photo, bio, specialties as tags, years in the trade and their Instagram. Reached from the **Services header**, next to the gallery - it is a shop window, and inside the booking flow only someone who had already picked a service and tapped "Reservar" would find it, which is too late to be useful - and also **from the work area**, so staff can check from their own phone how a CV looks (read-only there: the arrow goes back to `/admin` and no appointment is offered, because booking lives in `/tabs` and their guard would bounce them). Beyond that, same screen: **from the booking flow** — and you come back with the person already selected, carrying over whatever service was chosen, which was the point — and **from the login screen**, via a link, for someone who has no account yet. That second entry is why the CV is public: `GET /api/peluqueros/publicos` is read **without a token** and publishes nothing from the account and no commission, and inside `/tabs` (which requires a session) only registered users would ever benefit, so the route also exists **outside the guards**, like the assistant's. The only difference between the two entries is where the buttons lead, decided by the session and not the route: with no account you cannot book, so the action is to sign in, and the back arrow does not point at a guarded route. And **the choice survives the login**: the button goes to `/auth/login?returnUrl=/tabs/agendar?peluqueroId=…`, carrying the service over if one was already chosen, so signing in lands straight on booking with that person selected. The return URL is built with `createUrlTree` rather than by concatenation, so the Router encodes the params and there is only one way to spell the same route. Only active profiles appear, **in the order the admin set**, with the name as tie-breaker; a profile with no CV shows just the name, and with no photo it renders the initial rather than a broken box
...
* For staff, the work area has its own tab bar: **schedule** (closing appointments from an action sheet, with the notes and the "won't count towards sales" warning right in the dialog) and **sales**. A barber sees neither the services management tab nor the users tab, and their routes bounce back to the work area in a single hop. Instead they get a **read-only «Servicios» tab**: the very screen a customer sees, with photos, prices, descriptions, duration and search, but without «Agendar» or the header shortcuts, which belong to the customer area and whose guard would bounce them. The admin does not get it, since their «Servicios» tab already is the management screen
* **Contact screen** with the salon's address, phone number and email. Phone and email are `tel:` and `mailto:` links, which Capacitor hands to the system dialler and mail client instead of opening them inside the WebView. **The address opens the phone's default maps app** (Google Maps, Waze or any other, or the system chooser when none is set as default) and, when there is no maps app at all, **Google Maps in the browser**. On the web it opens Google Maps in a new tab
...
* `guards/`: `authGuard`, `adminGuard` and `staffGuard` (ADMIN or PELUQUERO). The work area's door is `staffGuard` and admin screens repeat `adminGuard` on their own route: hiding a link is not security, but showing a locked door is a broken panel. All three distinguish **why** they bounce: with no session they go to the login carrying the destination in `returnUrl`, and a stylist reaching an admin-only screen goes to `/inicio` — they are not missing a session, they are missing the role, and sending them to the login would ask for credentials they already have. A client does return to the login from `staffGuard`: the whole panel is not for them and there is no screen of theirs to go to
* **The panel login lets the whole staff in, not just the admin.** It had been requiring `isAdmin()` since before the `PELUQUERO` role existed, so a stylist got "this account has no permissions" and was signed straight out: half of what was built for them — their schedule, their sales, the gallery and their CV — was unreachable on the web. Now anyone who is `isStaff()` gets in, and sign-in goes through `/inicio` rather than `/dashboard`, because the dashboard lives off `/api/estadisticas`, which is ADMIN-only: sending a stylist there was sending them to a 403
...
**782 Vitest tests** run in CI on every push, followed by production builds of both apps:

| Suite | Tests | Covers |
|-------|-------|--------|
| Admin + core (`npx ng test`) | 434 | Feature components (citas, bloqueos, usuarios, servicios, peluqueros, produccion, perfil, dashboard, galeria, auth), the closed-day date picker, the searchable list modal, client-side image resizing, receipt download, the assistant client, and every core service, guard and interceptor. What is tested about the `PELUQUERO` role is mostly what it does **not** do: never requests the user list (that 403 would take the appointments down), never shows the cash or delete buttons, and never has menu links its own guard would reject. **Configurable permissions** are covered both ways: switched off, neither «Pago manual» nor «Reprogramar» appear; switched on they do, and only the one that was switched on; and an ADMIN sees both with the whole matrix off, because they never go through it. The **gallery** covers the per-owner split: a hairdresser with no permissions sees it with no actions at all, with the own-photos one they manage theirs and not a colleague's (nor the ownerless ones), with the someone-else's one it is the other way round, and being able to edit grants no right to reorder. The **public CV** covers the editor shared by both screens (a tag with a comma is rejected before spending a request, the same tag in different case is not repeated, a new CV rebuilds the form — otherwise the modal would show the previous profile's — and saving sends the whole block with `null` for what was cleared, which is the only thing that deletes), «My profile» (an account with no profile does not see the block, without the permission it is read-only, and the CV photo is not the account avatar) and the panel's profile tab (the CV comes nested in `/gestion`, saves with its own button rather than with the profile, and the «Team» order lives in the profile because it displaces colleagues). **Modules** are covered for what a permission cannot do: with no payment method at all, «Pago manual» never appears **not even for an ADMIN**, the modal offers only the enabled methods with an existing one preselected, with commissions off the column disappears from sales and from the profile and saving sends no rate and touches no exceptions, with payments off no warning claims the appointment will not count (there it does), and on the modules screen itself: only what changed is sent, what is lost is spelled out before saving, and a parent with no enabled children - or a child under a disabled parent - is flagged as not applying. Plus the core service: before the first response everything counts as on (hiding it all would make a network glitch break the panel), that a menu entry governed by a module **disappears for an ADMIN too** (sales and gallery), that without the team module there is no CV tab on the profile and no block on "My profile" - and there it is not even requested, because the endpoint answers 409 - and **a sleeping backend never stops the app from starting** - it waits two seconds at most and the request stays alive to correct itself - plus **asking again when the page comes back to the foreground**, which is what makes switching a module off reach the phone without killing the app |
| Mobile (`cd mobile && npx ng test`) | 348 | Booking flow (incl. barber selector and disabled closed days), Stripe payment page, biometric login and token storage, camera and profile photo, PDF receipt (share on device, download in the browser), appointment history, contact screen, the assistant chat (per-status error mapping, history trimming), the work gallery and its authorship line, appointment closing with its paid/unpaid warnings, own sales and the staff comparison, configurable permissions in an appointment's action sheet, taking cash or bank-transfer payment from the phone (still offered once the appointment is closed, because the natural order is to close it and charge afterwards), and the tab routes and guards. The **«Team» screen**: it comes from the public route and not the token-gated listing, respects the order the ADMIN set without reordering on its own, writes the years in singular or plural and hides them at zero, builds the Instagram link from the stored handle, the round trip from booking does not lose the already-selected service, and **with and without an account**: with no session the button leads to the login **with the destination in `returnUrl`** (carrying the service over if one was chosen), the back arrow does not point at a guarded route, and the listing loads all the same because the endpoint needs no token. The **`returnUrl`** is covered both ways: a client lands where the URL said, through both doors (form and fingerprint), and a destination outside the app is **not obeyed** — no leading slash, protocol-relative, with a scheme, with a backslash, or looping back to the login. The **guards** are covered for all four that require a session saving the destination with its params, the root not being saved because it is not a destination, and a bounce by **role** saving nothing. Plus the per-area split: a client is never sent to the work area nor staff to the client one, but what belongs to neither (`/pago/:id`, `/equipo`, `/asistente`) works for both. The routes are checked for the public one existing outside `/tabs`, guard-free and before the wildcard; and the **login** is checked against its template for still carrying its two account-free exits (the team and the assistant), because the link **is** the mechanism: removing it would break neither the compiler nor any other test, and those routes would become unreachable. The **assistant** is checked for having a way out: with no account the arrow leads to the login, with a session to its own area (`/tabs` or `/admin`, whichever its guard will not bounce), and inside the tabs it is not drawn at all, because there the bottom bar already is the way out; against the template too, and for the same reason. **Modules**: with no payment method there is no «Cobrar» even for an ADMIN, the radios are only the enabled methods with the first preselected, closing an appointment with payments off carries no "will not count" warning, a `null` commission prints no "NaN €" and does not break the comparison total, and the payment route is closed by a guard because the URL can be typed by hand. The phase-7 modules are covered where they are **the only way in**: the two icons in the Services header are checked **against the template** (removing them would leave the gallery and the team unreachable), the login link to the team drops on its own while the assistant one stays, the sales tab disappears from the phone, and `/equipo` still requires no session even though it now carries its module guard. **«El equipo»** now also covers staff: they go back to `/admin` rather than to booking (their guard bounces that route) and are not offered an appointment, because for them the screen is a shop window. And of the **staff catalogue**: the tab is checked on the template because it is their only way in (it sits between «Mi producción» and «Perfil», stays there without the sales module, and the ADMIN bar does not change), the route loads the very screen the customer sees rather than a copy, management stays behind `adminGuard`, and on that screen staff see the same data without «Agendar» or the header icons, while the customer loses nothing. And of the **Contact address**: in the app it is handed to the system as a location so the default maps app opens it, and when there is none or the plugin fails it ends up in Google Maps in the browser, because tapping it must never do nothing; on the web Google Maps opens in a new tab **with no `await` before it**, because outside the user gesture the popup blocker would swallow it; what gets searched is the address on screen, with no stray comma when the street or the town is missing; and the tappable row is checked on the template, because the `(click)` is the mechanism |
